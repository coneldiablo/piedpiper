...
2. `Unsupervised similarity analysis`
   ...
   - может сохранять индексированные профили в SQLite и Qdrant через `services/ml_profile_store.py` и `services/qdrant_profile_store.py`
...
- similarity search в локальном SQLite-хранилище и векторном хранилище Qdrant
...
- `services/qdrant_profile_store.py`
...
Это позволяет системе одновременно держать локальную базу профилей в SQLite и векторный индекс в Qdrant для similarity search.
